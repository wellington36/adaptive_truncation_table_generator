...
real log_Z_com_poisson_bounding(real log_mu, real nu) {
  real log_Z;
  int k = 2;
  int M = 300000;
  real leps = log(2^-52);
  ...
  print(log_mu, " | ", nu, " | ", M * log_mu - nu*lgamma(M + 1));
  ...
  while (log(exp(log_Z_terms[k]) * abs(1 - pow(1 - exp(log_Z_terms[k])/exp(log_Z_terms[k-1]), -1))) >= log(2) + leps && k < M) {
    ...
    //print(exp(log_Z_terms[k]) * abs(1 - pow(1 - exp(log_Z_terms[k])/exp(log_Z_terms[k-1]), -1)));
  }
  log_Z = log_sum_exp(log_Z_terms[1:k]);
  
  print("iterations of bounding: ", k, "| log_Z: ", log_Z, "| acresimo: ", log_Z_terms[k] - log(2));
...
real com_poisson_log_lpmf(int y, real log_mu, real nu) {
  ...
  return y * log_mu - nu*lgamma(y + 1) - log_Z_com_poisson_bounding(log_mu, nu);
}
// COM Poisson log-PMF for a single response
real com_poisson_lpmf_bounding(int y, real mu, real nu) {
...
real com_poisson_lcdf(int y, real mu, real nu) {
  ...
  log_Z = log_Z_com_poisson_bounding(log_mu, nu);
  if (y == 0) {
    return -log_Z; 
  }
  // first 2 terms of the series
