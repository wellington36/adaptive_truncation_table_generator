...
from mpmath import mp, mpf, log, exp, loggamma, binomial
...
def ratio_threshold(phi, mu, x, eta, eps, M, L, brute):
    ...
    print(f"Threshold: k={k} {exp(aprox)}")
...
if __name__ == "__main__":
        ...
        for mu in mul:
            for x in xl:
                for eta in etal:
                    ...
                    for eps in epsl:
                        if L < 1/2:
                            print(f"Evaluating: theta=({phi, mu, x, eta}), eps={float(eps)}...")
    ...
    Steps = len(phil) * len(mul) * len(xl) + len(etal) + len(epsl)

    ratios_bounding_less = [x/Steps for x in ratios_bounding_less]
    ratios_threshold_less = [x/Steps for x in ratios_threshold_less]
    ratios_cap_m_less = [x/Steps for x in ratios_cap_m_less]
    ratios_cap_M_less = [x/Steps for x in ratios_cap_M_less]
    ratios_bounding_great = [x/Steps for x in ratios_bounding_great]
    ratios_threshold_great = [x/Steps for x in ratios_threshold_great]
    ratios_cap_m_great = [x/Steps for x in ratios_cap_m_great]
    ratios_cap_M_great = [x/Steps for x in ratios_cap_M_great]
