...
for (i in seq_along(lambda)) {
  ...
  brute_64bits_error[i] <- abs(exp(log_diff_exp(log_Z_brute(log(lambda[i]), nu[i], M[i]), log_Z)))
}

bounding_64bits_error <- numeric(length(lambda))
for (i in seq_along(lambda)) {
  log_Z <- log_Z_value_bp(log(lambda[i]), nu[i], M[i], 2^-52*10^4)
  bounding_64bits_error[i] <- abs(exp(log_diff_exp(log_Z_brute(log(lambda[i]), nu[i], M[i]), log_Z)))
}
...
table <- data.frame(
  ...
  Column5 = brute_64bits_error,
  Column6 = bounding_64bits_error
)
